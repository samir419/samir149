...
opponents.forEach((opp,index)=>{
    ...
    document.getElementById('opponents').innerHTML+=`<div class="container-horizontal"><img src="${opp.battle_img}"><p>${opp.name}</p><button onclick="challenge_opp(${index});this.textContent='selected'">challenge</button></div>`
})
function challenge_opp(i){
    let opp = opponents[i]
    console.log('selected'+opp)
    if(opp.level>sessiondata.level){
...
let game = new DoubleGame([player1.ent,player2.ent])
let reward_amount=0